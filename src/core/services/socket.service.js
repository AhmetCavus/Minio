class SocketService {
  ...
  notifyAddCollectionItem = (schema, item) =>
    this.socketEngine.notifyAddCollectionItem(schema, item)

  notifyRemoveCollectionItem = (schema, item) =>
    this.socketEngine.notifyRemoveCollectionItem(schema, item)

  notifyUpdateCollection = (schema, items) =>
    this.socketEngine.notifyUpdateCollection(schema, item)

  notifyUpdateCollectionItem = (schema, item) =>
    this.socketEngine.notifyUpdateCollectionItem(schema, item)

  findSocket = (token, address) => this.socketEngine.findSocket(token, address)

  closeConnection = (token, address) =>
    this.socketEngine.closeConnection(token, address)
